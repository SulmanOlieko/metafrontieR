...
mf_sfa_ordonnell <- function(Xvar, groupFrontierMat, S, method, udist, ...) {
  yMeta <- apply(groupFrontierMat, 1, max, na.rm = TRUE)
  mf_sfa_fit(Xvar = Xvar, yMeta = yMeta, S = S, method = method,
              udist = udist, ...)
}
...
mf_huang <- function(Xvar, yhat_group, S, method, udist, ...) {
  mf_sfa_fit(Xvar = Xvar, yMeta = yhat_group, S = S, method = method,
              udist = udist, ...)
}
...
mf_sfa_fit <- function(Xvar, yMeta, S, method, udist, ...) {
  ...
  sfaObj <- tryCatch(
    sfacross(formula = fml, udist = udist, S = S, data = df_meta,
             method = method, ...),
    error = function(e) {
